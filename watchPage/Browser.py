...
class Browser:
    ...
    def getFormField(self, fieldName):
        """获取form的字段值"""
        if self.selectedForm is not None:
            if fieldName in self.selectedForm['fields']:
                return self.selectedForm['fields'][fieldName]
        return ''

    def setFormField(self, fieldName, value):
        """设置当前表单中字段的值"""
        self.selectedForm['fields'][fieldName] = value
    ...
    def requestAjaxData(self,url,data,referer=None,**headers):
        ...
        response = urllib2.urlopen(req, params, self.timeout)
